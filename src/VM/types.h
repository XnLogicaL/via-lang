...
struct via_Value
{
    ...
    via_Value() : type(VType::Nil), nil(nullptr) {}
    via_Value(const via_String& c) : type(VType::String), str(strdup(c)) {}

    explicit via_Value(const via_Number& d) : type(VType::Number), num(d) {}
    explicit via_Value(const via_Bool& b) : type(VType::Bool), boole(b) {}
    explicit via_Value(const via_Ptr& p) : type(VType::Ptr), ptr(p) {}
    explicit via_Value(const via_Table& t);
    explicit via_Value(const via_Func& f) : type(VType::Func), fun(new via_Func(f)) {}
    explicit via_Value(via_CFunc cf) : type(VType::CFunc), cfun(via_CFunc(cf)) {}
    explicit via_Value(const via_TableKey& tk) : type(VType::TableKey), tblkey(new via_TableKey(tk)) {}
