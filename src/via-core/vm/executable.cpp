...
template <>
void via::detail::ir_lower_expr<ir::ExprSymbol>(Executable& exe, const ir::ExprSymbol* ir_expr_symbol, u16 dst) noexcept
{
...
template <>
void via::detail::ir_lower_expr<ir::ExprBinary>(Executable& exe, const ir::ExprBinary* ir_expr_binary, u16 dst) noexcept
{
    ...
    if (opid >= static_cast<u16>(BinaryOp::ADD) && opid <= static_cast<u16>(BinaryOp::MOD)) {
    ...
    else if (opid >= static_cast<u16>(BinaryOp::AND) && opid <= static_cast<u16>(BinaryOp::OR)) {
        /* TODO: Check if rhs is constexpr, in which case increment base by one
         * for K instructions*/
        u16 base = static_cast<u16>(OpCode::AND) + static_cast<u16>(ir_expr_binary->op);
        exe.push_instruction(static_cast<OpCode>(base), {dst, rlhs, rrhs});
    }
    else if (opid >= static_cast<u16>(BinaryOp::BAND) && opid <= static_cast<u16>(BinaryOp::BSHR)) {
        /* TODO: Check if rhs is constexpr, in which case increment base by one
         * for K instructions*/
...
template <>
void via::detail::ir_lower_stmt<ir::StmtVarDecl>(Executable& exe, const ir::StmtVarDecl* ir_stmt_var_decl) noexcept
{
...
template <>
void via::detail::ir_lower_stmt<ir::StmtBlock>(Executable& exe, const ir::StmtBlock* ir_stmt_block) noexcept
{
...
template <>
void via::detail::ir_lower_stmt<ir::StmtFuncDecl>(Executable& exe, const ir::StmtFuncDecl* ir_stmt_func_decl) noexcept
{
    u16 dst = exe.m_reg_state.alloc();
...
template <>
void via::detail::ir_lower_stmt<ir::StmtExpr>(Executable& exe, const ir::StmtExpr* ir_stmt_expr) noexcept
{
...
via::Executable* via::Executable::build_from_ir(Module* module, const IRTree& ir, u64 flags) noexcept
{
...
void via::Executable::lower_jumps() noexcept
{
    for (usize pc = 0; Instruction & instr: m_bytecode) {
        ...
        if (opid >= static_cast<u16>(OpCode::JMP) && opid <= static_cast<u16>(OpCode::JMPIFX)) {
...
std::string via::Executable::get_dump() const
{
    ...
    oss << ansi::format("[section .text]\n", ansi::Foreground::Yellow, ansi::Background::Black, ansi::Style::Underline);

    for (usize pc = 0; const Instruction& insn: m_bytecode) {
        if (auto it = std::find_if(m_labels.begin(), m_labels.end(), [&pc](auto&& p) { return p.second == pc; });
            it != m_labels.end())
            oss << " .LB" << it->first << ":\n";
        oss << "  " << insn.get_dump() << "\n";
        pc++;
