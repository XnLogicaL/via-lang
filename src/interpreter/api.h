...
const TValue& get_table(TTable* tbl, uint32_t key);

// Sets the given tables corresponding hashed key field to a given value.
void set_table(TTable* tbl, uint32_t key, const TValue& value);
...
const TValue& get_global(State& state, uint32_t hash);

// Sets the global that corresponds to a given hashed identifier to a given value.
void set_global(State& state, uint32_t hash, const TValue& value);
