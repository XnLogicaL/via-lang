...
std::string AST::stringify_type_node(const TypeNode &type)
{
    ...
    {
        return std::format("UnionTypeNode(L: {}, R: {})", stringify_type_node(*union_node->lhs), stringify_type_node(*union_node->rhs));
    }
    else if (auto variant_node = std::get_if<VariantTypeNode>(&type))
    {
        return std::format(
            "VariantTypeNode(Variants: {})",
            format_vector<TypeNode *>(
                variant_node->types,
                [](const TypeNode *t)
                {
    ...
    {
        return std::format("TableTypeNode(Type: {})", stringify_type_node(*table_node->type));
    }
...
std::string AST::stringify_expr_node(const ExprNode &expr)
{
    ...
    {
        return std::format("LitExprNode(Val: {})", lit_expr->val.to_string());
    }
    else if (auto un_expr = std::get_if<UnExprNode>(&expr))
    {
        return std::format("UnExprNode(Expr: {})", stringify_expr_node(*un_expr->expr));
    }
    else if (auto group_expr = std::get_if<GroupExprNode>(&expr))
    {
        return std::format("GroupExprNode(Expr: {})", stringify_expr_node(*group_expr->expr));
    }
    else if (auto bin_expr = std::get_if<BinExprNode>(&expr))
    {
        return std::format(
            "BinExprNode(Op: {}, L: {}, R: {})", bin_expr->op.to_string(), stringify_expr_node(*bin_expr->lhs), stringify_expr_node(*bin_expr->rhs)
        );
    }
...
{
    return std::format(
        "LocalDeclStmt(Ident: {}, Type: {}, Const: {}, Val: {})",
...
{
    return std::format(
        "GlobalDeclStmtNode(Ident: {}, Type: {}, Val: {})", node.ident.to_string(), stringify_type_node(*node.type), stringify_expr_node(*node.val)
    );
}
...
{
    return std::format(
        "CallStmtNode(Ident: {}, TypeArgs: {}, Args: {})",
        node.ident.to_string(),
        format_vector<TypeNode *>(
            node.type_args,
            [](const TypeNode *t)
