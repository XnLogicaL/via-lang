...
void ExprNodeVisitor::visit(NodeBinExpr& binary_node, operand_t dst) {
  ...
  bool is_left_constexpr = is_constant_expression(ctx.lctx, lhs);
  bool is_right_constexpr = is_constant_expression(ctx.lctx, rhs);
  ...
  if (is_left_constexpr && is_right_constexpr && !is_bool_or_relational) {
    ...
    if (base_opcode == AND || base_opcode == OR) {
      ...
      else if (std::get_if<std::monostate>(&literal.value)) // 'nil' is always falsy
        is_rhs_falsy = true;
  ...
  non_constexpr:
    ...
    if (is_bool_or_relational) {
      operand_t right = alloc_register(ctx);
      bytecode_emit(ctx, MOV, {right, dst});
      bytecode_emit(ctx, base_opcode, {dst, reg, right});
      free_register(ctx, reg);
    }
    else {
...
void ExprNodeVisitor::visit(NodeArrExpr& array_expr, operand_t dst) {
  ...
  if (is_constant_expression(ctx.lctx, &array_expr)) {
...
void ExprNodeVisitor::visit(NodeIntrExpr& intrinsic_expr, operand_t dst) {
  ...
  if (intrinsic_expr.intrinsic.lexeme == "nameof") {
    ...
    if (NodeSymExpr* sym_expr = dynamic_cast<NodeSymExpr*>(target)) {
      ...
      auto comment = std::format("nameof({})", sym_expr->identifier.lexeme);
