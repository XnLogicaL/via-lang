...
struct CacheFile {
    ...
    const U64         magic_value = 0xDEADBEEFULL; // 8 bytes
    const U32         version;                     // 4 bytes
    const U64         compilation_date;            // 8 bytes
    U8                file_hash[32];               // 32 bytes (SHA-256)
    const char *const platform_info;               // 8 bytes (e.g., "x86_64-linux")
    const char *const runtime_flags;               // 8 bytes (e.g., "-O3")
